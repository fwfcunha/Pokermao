...
val networkModule = module {
    ...
    single { createNetworkClient(get(), get(named("baseUrlPokemon"))).create(PokemonService::class.java) }
    //
    // single { createNetworkClient(get(), get(named("baseUrlDigimon"))).create(DigimonService::class.java) }
    single { createOkhttpClientAuth(get()) }
    single(named("baseUrlPokemon")) {  "https://pokedexdx.herokuapp.com"}
    single(named("baseUrlDigimon")) {  "https://digimondx.herokuapp.com"}
}
